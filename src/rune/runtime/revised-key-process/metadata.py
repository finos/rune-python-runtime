...
from typing import Any, Never, get_args, Iterable, Tuple as TypingTuple
from typing_extensions import Self
from pydantic import PlainSerializer, PlainValidator, WrapValidator, WrapSerializer
...
class Reference:
    ...
    def __init__(
        ...
        if not isinstance(target, BaseMetaDataMixin) and ext_key:
            raise ValueError(
                "Need to pass an object as target when specifying an external key!"
            )
        if ext_key:
            ...
            # REVISED LOGIC: Check for existing key first
            existing = target.get_existing_key()
            if existing:
                self.target_key, discovered_type = existing
                self.key_type = key_type or discovered_type
                self.target = target
            else:
                if key_type and key_type != KeyType.INTERNAL:
                    raise ValueError(
                        "key_type should be None or INTERNAL when "
...
class BaseMetaDataMixin:
    ...
    def get_existing_key(self) -> TypingTuple[str, KeyType] | None:
        """REVISED: checks if the object already has any key assigned"""
        # Check internal first
        if key := self.get_meta("key"):
            return key, KeyType.INTERNAL
        # Check external
        if key := self.get_meta("key_external"):
            return key, KeyType.EXTERNAL
        # Check scoped
        if key := self.get_meta("key_scoped"):
            return key, KeyType.SCOPED
        return None
    ...
    def set_external_key(self, key: str, key_type: KeyType):
        ...
        if aux and aux != key:
            raise ValueError(
                f"This object already has an external key {aux}!"
    ...
    def _merged_allowed_meta(self, allowed_meta: set[str] | Iterable[str]) -> set[str]:
    ...
    def _check_props_allowed(self, props: dict[str, Any]):
        ...
        if not prop_keys.issubset(allowed):
            raise ValueError(f"Not allowed metadata provided: {prop_keys - allowed}")
    ...
    def _init_meta(self, allowed_meta: set[str]):
        ...
        if not current_meta.issubset(allowed_meta):
            raise ValueError(
                f"Allowed meta {allowed_meta} differs from the "
    ...
    def _bind_property_to(self, property_nm: str, ref: Reference):
        ...
        if ref.key_type.rune_ref_tag not in allowed_ref_types.get(
            property_nm, {}
        ):  # and not _replaceable(old_val)):
            raise ValueError(
                f"Ref of type {ref.key_type} "
        ...
        if not (isinstance(allowed_type, str) or isinstance(ref.target, allowed_type)):
            raise ValueError(
                "Can't set reference. Incompatible types: "
        ...
        if property_nm not in refs:
            ...
            if not _replaceable(old_val):
                raise ValueError(
                    f"Property {property_nm} of type "
    ...
    def _update_object_maps(self, new_maps):
        ...
        for map_type, new_map in new_maps.items():
            ...
            if dup_keys := set(local_map.keys()).intersection(set(new_map.keys())):
                raise ValueError(
                    "Duplicated keys detected in updating the "
    ...
    def _get_rune_scope_type(cls):
        ...
        """
        try:
            module = importlib.import_module(cls.__module__.split(".", maxsplit=1)[0])
...
class ComplexTypeMetaDataMixin(BaseMetaDataMixin):
    ...
    def deserialize(cls, obj, allowed_meta: set[str]):
        ...
        if not isinstance(obj, dict):
            raise PydanticCustomError(
                ...
                "Expected either {my_type} or dict but got {type}.",
    ...
    def validator(cls, allowed_meta: tuple[str] | tuple[Never, ...] = tuple()):
        ...
        return PlainValidator(
            partial(cls.deserialize, allowed_meta=allowed), json_schema_input_type=dict
        )
...
class BasicTypeMetaDataMixin(BaseMetaDataMixin):
    ...
    _INPUT_TYPES: Any | TypingTuple[Any, ...] = str  # to be overridden by subclasses
    ...
    def _check_type(cls, value):
        if not isinstance(value, cls._INPUT_TYPES):
            raise ValueError(
                f"{cls.__name__} can be instantiated only with "
    ...
    def validator(cls, allowed_meta: tuple[str]):
        ...
        return WrapValidator(
            partial(cls.deserialize, base_types=cls._INPUT_TYPES, allowed_meta=allowed),
...
class EnumWithMetaMixin:
    ...
    def deserialize(cls, obj, allowed_meta: set[str]):
        """method used as pydantic `validator`"""
        model = obj
        if isinstance(obj, str) and not isinstance(obj, _EnumWrapper):
            model = _EnumWrapper(cls(obj))  # type: ignore
        if isinstance(obj, EnumWithMetaMixin) and not isinstance(obj, _EnumWrapper):
    ...
    def validator(cls, allowed_meta: tuple[str] | tuple[Never, ...] = tuple()):
        ...
        allowed = set(allowed_meta)
        return PlainValidator(
            partial(cls.deserialize, allowed_meta=allowed),
