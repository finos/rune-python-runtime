...
class BaseDataClass(BaseModel, ComplexTypeMetaDataMixin):
    ...
    model_config = ConfigDict(
        extra="ignore", revalidate_instances="always", arbitrary_types_allowed=True
    )

    def __init__(self, **data: Any) -> None:
        """Constructor that captures and applies metadata if provided"""
        metadata_args = {}
        valid_fields = {}

        # Separate metadata keys from standard Pydantic fields
        for k, v in data.items():
            if k in (self.model_fields if hasattr(self, "model_fields") else {}):
                valid_fields[k] = v
            elif k.startswith("key"):  # captures 'key', 'key_external', 'key_scoped'
                metadata_args[k] = v
            else:
                valid_fields[k] = v

        # Initialize the Pydantic model
        super().__init__(**valid_fields)

        # Apply and register any metadata provided at construction time
        if metadata_args:
            self.set_meta(check_allowed=True, **metadata_args)
            self._register_keys(self._get_meta_container())
    ...
    def __setattr__(self, name: str, value: Any) -> None:
        if isinstance(value, Reference):
            ...
            if isinstance(self.__dict__[name], _EnumWrapper) and not isinstance(
                value, _EnumWrapper
            ):
    ...
    def _init_rune_parent(self):
        ...
        for prop_nm, obj in self.__dict__.items():
            if (
                isinstance(obj, BaseMetaDataMixin)
                and not prop_nm.startswith("__")
                and prop_nm not in refs
            ):
    ...
    def rune_deserialize(
        ...
        if isinstance(rune_data, str):
            ...
            raise ValueError(
                f"rune_data is of type {type(rune_data)}, "
    ...
    def resolve_references(self, ignore_dangling=False, recurse=True):
        ...
        if recurse:
            for prop_nm, obj in self.__dict__.items():
                if isinstance(obj, BaseDataClass) and not prop_nm.startswith("__"):
                    obj.resolve_references(
                        ignore_dangling=ignore_dangling, recurse=recurse
                    )
    ...
    def validate_conditions(
        ...
        if recursively:
            for k, v in self.__dict__.items():
                ...
                exceptions += _validate_conditions_recursively(v, raise_exc=raise_exc)
        err = f"with {len(exceptions)}" if exceptions else "without"
        logging.info("Done conditions checking for %s %s errors.", self_rep, err)
        return exceptions
    ...
    def add_to_list_attribute(self, attr_name: str, value) -> None:
        ...
        if not isinstance(value, allowed_types):
            raise TypeError(
                f"Value must be an instance of {allowed_types}, not {type(value)}"
            )
